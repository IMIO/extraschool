...
Import des données
******************
Il est possible d'importer les fiches signalétiques des enfants et des parents venant de fichiers xls.
...
* Format d'export
	...
	* Demander aux directeurs d'écoles de respecter le même format de fichier donné par le contact Imio
		
		* Ordre des colonnes
		* Libellé des colonnes

Il faut  impérativement un fichier par école. Sans cela, l'importation ne pourra se faire.
Voici l'exemple pour le fichier d'importation :
...
Les champs obligatoires de cette fiche sont :

* Nom
* Adresse

Les autres informations inscrites sur la fiche sont à titre informatif.
...
Lieu d'accueil
**************
Le lieu d'accueil est l'endroit où se déroule une activité, il peut être partagé par plusieurs implantations scolaires.
...
Parent
******
Personne responsable à qui les différents documents seront adressés
...
* Réduction en fonction de la position de l'enfant
	...
	* Unité de temps
		...
		* ...
	* Tolérance :
		Il est possible de paramètrer une tolérence exprimée en minute pour le calcul de la durée.
		
		| ex. unité : 30 minutes, tolérance 5 minute
		|	durée mesurée 34 minutes -> durée facturée 1*30 minutes
		|	durée mesurée 36 minutes -> durée facturée 2*30 minutes
		
	* Avez-vous un système de pénalité pour les retards, si oui lequel ?
	* Avez-vous des plages horaires gratuites ?
	* Avez-vous des périodes d'étude qui doivent être décomptées  ?
...
* Le smartphone est dédié à la gestion des présences
...
Le smartphone s'utilise sans connection Wi-Fi ou 3G,
cependant il faut absolument une connexion une fois par jour pour transmettre les données au serveur.
 	...
 	* wifi :
 		...
 		* Certains directeurs d'école sont contre pour des raisons de santé.
 			...
 			Dans ce cas, il possible d'utiliser une prise avec minuteur qui active le Wi-Fi la nuit pour la transmission
...
Il faut un QR Code par enfant si ces parents ne sont pas divorcés.
