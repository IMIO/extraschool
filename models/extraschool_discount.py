...
class extraschool_discount(models.Model):
    ...
    #                 if line.is_max_price_related_to_activity:
    #                     if len(self.env["extraschool_prestationtime"].search(
    #                         [("prestation_date", "=", line.prestation_date),
...
class extraschool_discount_version(models.Model):
    ...
    price_list_ids = fields.Many2many('extraschool.price_list', 'extraschool_discount_pricelist_rel',
                                      string='Price list')
    child_type_ids = fields.Many2many('extraschool.childtype', 'extraschool_discount_childtype_rel',
                                      string='Child type')
    ...
    def compute(self, biller_id):
        ...
        if (self.discount_template == 'hannut'):
            ...
            invoice_line_ids = self.env['extraschool.invoicedprestations'].search(
                [('invoiceid', 'in', biller_id.invoice_ids.ids)])
            # filter on price list if needed
            if len(self.price_list_ids):
                invoice_line_ids = invoice_line_ids.filtered(
                    lambda r: r.price_list_version_id.price_list_id.id in self.price_list_ids.ids)
            ...
            invoice_to_compute = []
            for line_id in invoice_line_ids.sorted(key=lambda r: "%s%s%s" % (
                fields.Date.from_string(r.prestation_date).strftime("%V"), r.childid.id,
                r.activity_occurrence_id.activityid.id)):
                if fields.Date.from_string(line_id.prestation_date).strftime(
                    "%V") != saved_week or line_id.childid.id != saved_child or line_id.activity_occurrence_id.activityid.id != saved_activity:
