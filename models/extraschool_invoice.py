...
class extraschool_invoice(models.Model):
    ...
    name = fields.Char('Name', size=20,readonly=True, default='Facture', track_visibility=True)
    schoolimplantationid = fields.Many2one('extraschool.schoolimplantation', 'School implantation', required=False,readonly=True, index=True, track_visibility=True)
    classid = fields.Many2one('extraschool.class', 'Class', required=False, domain="[('schoolimplantation','=',schoolimplantationid)]", index=True, track_visibility=True)
    parentid = fields.Many2one('extraschool.parent', 'Parent', required=False, index = True, track_visibility=True)
    invoicesendmethod = fields.Selection(related="parentid.invoicesendmethod", store=True, track_visibility=True)
    number = fields.Integer('Number',readonly=True, track_visibility=True)
    structcom = fields.Char('Structured Communication', size=50, readonly=True, required=True, track_visibility=True)
    amount_total = fields.Float(string='Amount', digits=dp.get_precision('extraschool_invoice'), readonly=True, store=True, track_visibility=True)
    amount_received = fields.Float( string='Received', digits=dp.get_precision('extraschool_invoice'), readonly=True,store=True, track_visibility=True)
    balance = fields.Float(digits=dp.get_precision('extraschool_invoice'), string='Balance',readonly=True, store=True, track_visibility=True)
    no_value = fields.Float('No value',default=0.0,readonly=True, track_visibility=True)
    discount = fields.Float('Discount',readonly=True, track_visibility=True)
    biller_id = fields.Many2one('extraschool.biller', 'Biller', required=False,ondelete='cascade',readonly=True, index=True, track_visibility=True)
    filename = fields.Char('filename', size=20,readonly=True,  invisible= True, track_visibility=True)
    invoice_file = fields.Binary('File', readonly=True, invisible= True, track_visibility=True)
    payment_ids = fields.One2many('extraschool.payment_reconciliation', 'invoice_id','Payments', track_visibility=True)
    invoice_line_ids = fields.One2many('extraschool.invoicedprestations', 'invoiceid','Details', track_visibility=True)
    refound_line_ids = fields.One2many('extraschool.refound_line', 'invoiceid','Refound', track_visibility=True)
    oldid = fields.Char('oldid', size=20, track_visibility=True)
    activitycategoryid = fields.Many2many('extraschool.activitycategory', 'extraschool_invoice_activity_category_rel', store=True, auto_join=True, track_visibility=True)
    # activitycategoryid = fields.Many2one(related='biller_id.activitycategoryid', store=True, auto_join=True, track_visibility=True)
    period_from = fields.Date(related='biller_id.period_from', index=True, track_visibility=True)
    period_to = fields.Date(related='biller_id.period_to', index=True, track_visibility=True)
    payment_term = fields.Date('Payment term', track_visibility=True)
    comment = fields.Text("Comment",default="", track_visibility=True)
    last_reminder_id = fields.Many2one('extraschool.reminder', 'Last reminder',readonly=True, index = True, track_visibility=True)
    reminder_fees = fields.Boolean('Reminder fees', default=False, track_visibility=True)
    huissier = fields.Boolean('Huissier', default=False, track_visibility=True)
    fees_huissier = fields.Float('Fees Huissier', default=0.0, track_visibility=True)
    tag = fields.Many2one('extraschool.invoice_tag', 'Tag', readonly=True, track_visibility=True)
