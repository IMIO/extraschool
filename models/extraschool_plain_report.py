...
class extraschool_plain_report(models.Model):
    ...
    _inherit = 'extraschool.subvention_report'

    activity_plain = fields.Many2many(
    ...
    def _generate_subvention_request(self, document):
        ...
        self.document.add_heading('Centre de Vacances', 0)
        self.document.add_heading('Formulaire de demande de subventionnement', 1)
        for section in sections:
            self.document.add_heading(section[0], 2)
            paragraph = self.document.add_paragraph()
            for i in range(1, len(section)):
                paragraph.add_run(section[i].decode('utf-8')).bold = True

        self.document.add_heading(u'REMARQUE IMPORTANTE')
        self.document.add_paragraph(u'Pour être recevable, la présente demande doit impérativement comprendre :')
        self.document.add_paragraph(u'la liste des enfants accueillis ; ').style = 'List Bullet'
        self.document.add_paragraph(u'la liste du personnel d’encadrement (animateurs, coordinateur(s), responsable qualifié, animateurs ou coordinateur(s) en 2ème stage de formation), accompagnée des demandes d’assimilation s’il s’agit de la première prestation de l’animateur ou du coordinateur concerné ; ').style = 'List Bullet'
        self.document.add_paragraph(u'le tableau de présences journalières (enfants et animateurs) ; ').style = 'List Bullet'
        self.document.add_paragraph(u'e cas échéant, le justificatif de versement des indemnités aux animateurs et coordinateurs, sous la forme d’une déclaration sur l’honneur. ').style = 'List Bullet'
        self.document.add_paragraph().add_run(u'Sans ces annexes complètes, il ne peut être procédé à l’examen du droit à la subvention ni au calcul de celle-ci ! ').bold = True
        paragraph = self.document.add_paragraph()
    ...
    def create(self, vals):
        ...
        return super(extraschool_plain_report, self).create(vals)
