...
class extraschool_prestationtimes(models.Model):
    ...
    childid = fields.Many2one('extraschool.child', 'Child', domain="[('isdisabled','=',False)]", required=False, index=True, ondelete='restrict')
    parent_id = fields.Many2one(related='childid.parentid', store=True, index=True)
    prestation_date = fields.Date('Date', index=True, Index=True)
    prestation_time = fields.Float('Time', index=True, Index=True, required=True)
    ...
    verified = fields.Boolean('Verified',default=False, index=True)
    error_msg = fields.Char('Error', size=255)
    activity_occurrence_id = fields.Many2one('extraschool.activityoccurrence', 'Activity occurrence', index=True)
    activity_name = fields.Char(related='activity_occurrence_id.activityname')
    activity_category_id = fields.Many2one('extraschool.activitycategory', 'Activity Category', required=True, index=True)
