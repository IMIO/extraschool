...
class extraschool_payment_status_report(models.Model):
    ...
    activity_category_id = fields.Many2one('extraschool.activitycategory', index=True)
    parent_id = fields.Many2one('extraschool.parent', index=True)
    solde = fields.Float('solde', index=True)
...
class extraschool_payment_report(models.Model):
    ...
    parent_id = fields.Many2one('extraschool.parent', index=True)
    amount = fields.Float('amount', index=True)
    solde = fields.Float('solde', index=True)
...
class extraschool_aged_balance(models.TransientModel):
    ...
    aged_date = fields.Date( string ='Reconcil Date', index=True)
