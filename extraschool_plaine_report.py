...
class extraschool_plaine_report(models.Model):
    ...
    @api.multi
    def generate_plaine(self):


        if self.validity_from > self.validity_to:
            raise Warning(_("La date de fin doit être plus grande que la date de début !"))

        report = self.env['report']._get_report_from_name('extraschool.tpl_plaine_report_wizard_report')
        ...
        return {
            ...
            'report_name': 'extraschool.tpl_plaine_report_wizard_report',
            'datas': datas,
